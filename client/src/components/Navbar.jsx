    ...
    return (
        <>
            ...
            <nav style={{
                ...
                {isHome && (
                    <div className="nav-desktop-links" style={{ gap: '1.5rem' }}>
                        {['Tracks', 'Schedule', 'FAQ'].map(label => (
                            <a key={label} href={`#${label.toLowerCase()}`}
                                style={{ fontFamily: 'DM Mono,monospace', fontSize: '.72rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--muted)', textDecoration: 'none', transition: 'color .2s' }}
                ...
                {/* Desktop right CTAs */}
                <div className="nav-desktop-links" style={{ alignItems: 'center', gap: '1rem' }}>
                    <Link to="/login"
                        style={{ fontFamily: 'DM Mono,monospace', fontSize: '.75rem', letterSpacing: '.05em', color: 'var(--muted)', textDecoration: 'none', transition: 'color .2s' }}
                ...
                <button
                    ...
                    <span style={{
                        ...
                        background: open ? 'var(--orange)' : 'var(--text)',
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
                    {isHome && ['Tracks', 'Schedule', 'FAQ'].map(label => (
                        <a key={label} href={`#${label.toLowerCase()}`} onClick={close}
                            style={{
                                ...
                                color: 'var(--muted)', padding: '4px 0', textDecoration: 'none'
                            }}>
                            {label}
                        </a>
                    ))}
                    ...
                    <Link to="/login" onClick={close}
                        style={{
                            fontFamily: 'DM Mono,monospace', fontSize: '.82rem',
                            letterSpacing: '.1em', color: 'var(--orange)',
                            padding: '4px 0', textDecoration: 'none'
                        }}>
                        → Team Login
                    </Link>
                    <Link to="/register" className="btn-primary" onClick={close}
                        style={{ justifyContent: 'center', textAlign: 'center', marginTop: '.25rem' }}>
                        Register Now →
                    </Link>
                </div>
            </div>
